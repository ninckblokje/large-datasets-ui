<template>
  <div class="row">
    ...
    <div class="col-sm-11">
      ...
      <div class="btn-group">
        <button class="btn btn-outline-primary" :disabled="disablePreviousButton" @click="previousPage">Previous</button>
...
export default {
  ...
  computed: {
    disableNextButton() {
      return (this.pageIndex * this.pageSize) + this.pageSize >= this.data.length
    },
  ...
  methods: {
    getDataSet() {
      ...
      axios.get("http://localhost:8080/api/dataset", { params: { size: this.size } })
          .then(response => {
    ...
    populatePage(page) {
      this.pageIndex = page
      this.pageData = this.data.slice((page * this.pageSize), (page * this.pageSize) + this.pageSize)
    },
    previousPage() {
      this.populatePage(this.pageIndex - 1)
    },
